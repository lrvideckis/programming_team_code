...
doxygen | [Doxyfile](Doxyfile) | Used mainly as a style guide (not enforced by CI), and to check percentage of documented code (enforced by CI). The generated docs are not useful in CP and unavaliable in ICPC contests, so I don't generate them. To see changed settings: `doxygen -x Doxyfile` | https://www.doxygen.nl/manual/commands.html
