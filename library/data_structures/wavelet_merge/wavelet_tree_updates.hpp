...
struct wavelet_tree_updates {
     ...
     * @endcode
     * @param a,_minv,_maxv must satisfy minv <= a[i] < maxv
     ...
     */
    wavelet_tree_updates(const vector<int>& a, int _minv, int _maxv, const vector<bool>& active) : n(ssize(a)), minv(_minv), maxv(_maxv), bool_presums(maxv - minv, vector<bool>()), bool_bits(2 * (maxv - minv), vector<bool>()) {
