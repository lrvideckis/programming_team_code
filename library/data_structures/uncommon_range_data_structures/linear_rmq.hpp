...
template <typename T> struct linear_rmq {
	...
	vector<vector<int>> block_mn; //block_mn[level][i] = index of min in range [i, i + 64^(level+1))
	...
	int min_idx_block(int level, int le, int ri) const {//returns index of min in range [le, ri)
		...
		return idx[level][le + __lg(mask[level][le] & ((1ULL<<(ri-le))-1))];
	}
	...
	int query_idx(int le, int ri) const {//time: theoretically O(log(n) / log(log(n))), practically 3 times complexity of normal RMQ query
		...
		int res = le;
		for(int level = 0; le < ri && level < ssize(mask); level++) {
