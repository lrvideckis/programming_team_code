...
struct wavelet_tree_updates {
    ...
    vector<bit_presum> presum;
    vector<bit_bit> bit;//TODO: better name?
    wavelet_tree_updates(vector<int> arr, int minv, int maxv) : N(ssize(arr)), MINV(minv), MAXV(maxv), orig_arr(arr), presum(MAXV - MINV, vector<bool>()), bit(2 * (MAXV - MINV), 0) {
    ...
    void set_active_impl(int i, bool is_active, int orig_value, int tl, int tr, int v) {
        bit[v].set(i, is_active);
        if (tr - tl == 1) return;
        int tm = split(tl, tr), pi = presum[v].popcount(i);
    ...
    int rect_count_impl(int le, int ri, int x, int y, int tl, int tr, int v) const {
        ...
        if (x <= tl && tr <= y) return bit[v].popcount(le, ri);
        int tm = split(tl, tr), pl = presum[v].popcount(le), pr = presum[v].popcount(ri);
    ...
    int kth_smallest_impl(int le, int ri, int k, int tl, int tr, int v) const {
        ...
        int tm = split(tl, tr), pl = presum[v].popcount(le), pr = presum[v].popcount(ri);
        int cnt_left = bit[2 * v].popcount(pl, pr);
